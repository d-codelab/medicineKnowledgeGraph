<template>
  <div>
    ...
    <div id="mode">
      ...
      <div class="gState" style="margin-top: 10px;margin-bottom: 10px;">
        ...
        <span
          ...
          style="border-top-right-radius:0;border-bottom-right-radius:0;width: 30px;"
        >显示</span>
        <!-- <span
        ...
        <span
          ...
          style="border-top-left-radius:0;border-bottom-left-radius:0;position:relative;left:-5px;width: 30px;"
        >隐藏</span>
      </div>
      <div class="gState" style="margin-bottom: 10px;">
        <p class="font-sky" style="text-align: left;">
          <strong>视图切换：</strong>
        </p>
        <span
          @click="changeView(0)"
          :class="{ active: is2dView }"
          style="border-top-right-radius:0;border-bottom-right-radius:0;width: 30px;"
        >2D</span>
        <!-- <span
          @click="changeNodeState(1)"
        ...
        <span
          ...
          style="border-top-left-radius:0;border-bottom-left-radius:0;position:relative;left:-5px;width: 30px;"
        >3D</span>
<!--        <el-button size="small" round icon="el-icon-refresh-right" type="info" plain @click="reload">重置</el-button>-->
      </div>
      <p class="font-sky" style="text-align: left;line-height: 1.5;margin-top: 0px;">
...
<!--        <br><br>-->
<!--        <strong style="font-size: 14px;">提示：2D视图下选中节点，<br>单击右键探索更多功能！</strong>-->
      </p>
    </div>
    <svg
      id="svg"
...
<!--        #409EFB-->
        <div slot="header" class="clearfix">
          ...
          <el-button
            @click="btnEdit"
            style="float: right; padding: 3px 0;color: #1A5276;font-size: 15px;"
            type="text"
          >查看详情</el-button>
        </div>
        <div
        ...
        >
          <span v-if="key !== 'cause' && key !== 'desc' && key !== 'prevent'"
                style="margin-right: 8px;">{{ (nodeObjMap[key] ? nodeObjMap[key] : key) + ':' }}</span>
          <span v-if="key !== 'cause' && key !== 'desc' && key !== 'prevent'"
                style="text-align: right;"><b>{{ item }}</b></span>
        </div>
      </el-card>
    </div>
    <!-- 编辑框 -->
  ...
  methods: {
    ...
    changeView (state){
      ...
      if (this.viewState !== state) {
        ...
        if (this.viewState === 1) {
          this.$router.push({name: '3dView'})
        } else {
          this.$router.push({name: '2dView'})
        }
      }
    },
    // 隐藏文字
    ...
    hideNodeOfType (event) {
      if (this.nodes.length === this.data.nodes.length
        ...
        // const nodeTypes = ['Disease', 'Department', 'Symptom', 'Food']
        // const linkTypes = ['', 'department_is', 'has_symptom', 'recommand_eat']
        // 图例的状态切换（对应类型的节点隐藏）
        if (state === 'on') {
          // 隐藏该类型的所有节点及关联关系
